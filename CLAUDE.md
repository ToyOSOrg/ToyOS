...
- **An agent never waits on CI.** It arms auto-merge, reports, and exits — a shell held open sleeping between checks is a stuck slot that looks like work. Sequencing across landings belongs to the orchestrator, done in passes on its own wake-ups; several finished branches land as one batch PR rather than as one agent babysitting N cycles.
