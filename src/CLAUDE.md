...
- **A job routed to the self-hosted runner must need the accelerator, the persistent cache, or that machine's CPU** — it has one worker, so a job that waits there for something another job on the same machine must produce cannot finish. Branch traffic (`pull_request`, `push`, merge-queue refs) is GitHub-hosted; the T14 serves the non-`ci.yml` schedules and `workflow_dispatch`; `.github/workflows/route.yml` is the one place that decision is written.
