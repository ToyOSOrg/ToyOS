...
fn merge_rlibs(rlib_paths: &[std::path::PathBuf]) -> Vec<u8> {
    ...
    for path in rlib_paths {
        ...
        // Refuse with the evidence. The bare assert this replaced named
        // neither the file nor what it held instead, so its one firing on a
        // hosted macOS runner left nothing to diagnose.
        assert!(
