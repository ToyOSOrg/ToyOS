...
//!
//! **A refusal hands the kernel nothing rather than something partial.** The
//! kernel places a BAR inside these windows and nowhere else, so a list missing
//! one window is worse than no list: an address outside every window firmware
//! decodes is unrouted, and a read of it answers all-ones exactly as an absent
//! device does.
...
use toyos_acpi::{list_len, memory_windows, Phys, MAX_LIST_BYTES};
use uefi::prelude::*;
...
/// Seventeen pointers, the `Configuration` pointer, and a `UINT32` padded to
/// the alignment of the pointers around it. A field added or dropped above
/// moves `configuration`, and the symptom is firmware calling something else.
...
///
/// Zero is the kernel knowing of no address below 4 GiB that reaches the bus,
/// which is what it then refuses on. It is never a guess.
pub fn windows(system_table: &SystemTable<Boot>, out: &mut [RootBridgeWindow]) -> usize {
    ...
    for (index, handle) in handles.iter().enumerate() {
        ...
        // every window below and the fixture the decoder's own tests read.
        //
        ...
        println!("{HEAD} {index} (segment {}) {len} bytes: {hex}", bridge.segment_number);
    ...
    for window in &out[..found] {
        println!("{HEAD} mem {:#x}..{:#x}", window.base, window.end());
    }
    if found == 0 {
