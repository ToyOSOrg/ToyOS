...
/// Collect all leaf entries by iterating the entire tree — for a caller whose
/// walk is its own (`delete_prefix`); a boundary-crossing listing goes through
/// [`collect_up_to`], whose ceiling sizes the allocation.
pub fn collect_all(io: &dyn BlockIO, root: BlockNum) -> Result<Vec<Entry>, FsError> {
    collect_up_to(io, root, usize::MAX)
}

/// Collect at most `limit` live leaf entries, refusing *before* the over-bound
/// entry lands: the tree's own claim about its size never reaches the allocator.
