...
/// How IPv4 identification fields are assigned across segments of a GSO frame.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum Ipv4IdMode {
    /// Every segment shares the same IPv4 identification field.
    Fixed,
    /// Each segment's IPv4 identification field increments by 1 from the
    /// preceding segment.
    Incrementing,
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct GsoInfo {
    /// The maximum segment size (payload length per segment) that GSO should
    /// use during segmentation.
    pub gso_size: core::num::NonZeroU16,
    /// How IPv4 identification fields are assigned across segments generated by
    /// GSO, or `None` for IPv6.
