...
//! attempts, as a stopping machine refuses every retry for a while. The judge
//! is `toyos-fat32-check`, written from fatgen103 and sharing no code with the
...
use spec_volume::{fat_offset, Volume, BYTES_PER_SECTOR, FAT_SECTORS, NUM_FATS};
use toyos_fat32::{BlockAccess, Error, Fat32, FatTime, File, IoError};
...
impl Faulty {
    fn new(bytes: Vec<u8>) -> Faulty {
        Faulty { bytes, writes: 0, plan: None, refused: 0 }
    }
    ...
    for outcome in [Outcome::Refused, Outcome::Landed] {
        for count in [1u64, 2, 3, 12] {
            for from in 0u64.. {
                ...
                let mut h = handle;
                fs.device().arm(burst(from, count, outcome));
                ...
                if count == 1 && !h.is_some_and(|f| f.needs_reconcile()) {
...
fn append() -> Scenario {
    Scenario { name: "append", setup: one_cluster_logged, call: append_two_clusters, verify: appended_in_place }
}
...
/// Erase, then free the chain. A refusal while erasing restores the entry; a
/// refusal once it is erased carries the free forward, so a retry answers
/// `NotFound` and nothing leaks either way.
fn remove() -> Scenario {
    Scenario {
        name: "remove",
        setup: |fs| {
        ...
        call: |fs, _, attempt| match fs.remove(DOOMED) {
            Err(Error::NotFound) if attempt > 0 => Ok(()),
            r => r,
        },
        verify: |fs, _| {
...
fn remove_dir() -> Scenario {
    Scenario {
        ...
        call: |fs, _, attempt| match fs.remove_dir("A directory with a long name") {
            Err(Error::NotFound) if attempt > 0 => Ok(()),
            r => r,
        },
        verify: |fs, _| {
...
/// orphaned clusters, at most one entry whose copies differ, a partial
/// long-name run — and the handle's own window a growth leaves until
/// `flush_meta`. Never a chain into free space, a cluster two entries reach,
/// or a second split entry.
#[test]
...
fn a_stop_that_refuses_every_retry_for_a_while_leaks_nothing() {
    ...
    for attempt in 2..=9 {
        ...
        assert_eq!(append_two_clusters(&mut fs, &mut h, attempt), Err(Error::BudgetExpired));
    }
    fs.device().plan = None;
