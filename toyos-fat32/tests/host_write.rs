...
/// The byte range of `cluster`'s entry in the active FAT: the write that links
/// a chain onto whatever follows `cluster`.
fn active_link_of(geom: &toyos_fat32::Geometry, cluster: u32) -> (u64, u64) {
    let cluster = geom.cluster(cluster).expect("a data cluster");
    let at = geom.fat_entry_offset(geom.active_fat.unwrap_or(0), cluster);
    (at, at + 4)
}

/// A one-cluster file, flushed and synced, on a device that refuses nothing
/// until armed; and the cluster holding its data.
fn one_cluster_file(
    image: &Image,
    bpc: usize,
) -> (Fat32<RefuseOnceInRange<common::BlockyFile>>, toyos_fat32::File, u32) {
    let mut fs = Fat32::mount(RefuseOnceInRange::new(image.device(), IoError::BudgetExpired))
        .expect("mount");
    let mut f = fs.create("log.bin", stamp()).expect("create");
    fs.write(&mut f, 0, &pattern(bpc, 93)).expect("the first cluster");
    fs.flush_meta(&mut f, stamp()).expect("flush");
    fs.sync().expect("sync");
    let geom = *fs.geometry();
    let extents = fs.extents("log.bin", 2).expect("extents");
    assert_eq!(extents.len(), 1, "log.bin was written as one cluster");
    let root = geom.root();
    let from_root = (extents[0].offset - geom.cluster_offset(root)) / bpc as u64;
    (fs, f, root.raw() + from_root as u32)
}

/// An append whose link write is refused after the cluster it links was
/// claimed must not leave that cluster claimed and unreached: the retry the
/// refusal invites links the same cluster.
///
/// The recorded real failure this reproduces: a T14's USB stick refused one
/// write to the active FAT mid-append, and the log partition it left ran the
/// file's chain 33 → 35 around an end-of-chain cluster 34 no entry reached.
#[test]
fn a_refused_link_is_redriven_onto_the_cluster_it_claimed() {
    let image = image("link-refusal-retried");
    let geom = *Fat32::mount(image.device()).expect("probe mount").geometry();
    let bpc = geom.bytes_per_cluster() as usize;
    let more = pattern(bpc, 94);
    {
        let (mut fs, mut f, last) = one_cluster_file(&image, bpc);
        fs.device().arm(active_link_of(&geom, last));
        let refused = fs.write(&mut f, bpc as u64, &more).expect_err("the link was armed to refuse");
        assert_eq!(refused, Error::BudgetExpired);

        fs.write(&mut f, bpc as u64, &more).expect("the retry after a spent budget");
        fs.flush_meta(&mut f, stamp()).expect("flush");
        fs.sync().expect("sync");
        common::assert_fats_agree(&mut fs);
        let mut want = pattern(bpc, 93);
        want.extend_from_slice(&more);
        assert_eq!(read_all(&mut fs, "log.bin"), want);
    }
    image.fsck();
}

/// The same refused link with no retry: the sync that ends the volume's use —
/// the shutdown's — gives the claim back.
#[test]
fn a_refused_link_never_retried_is_released_by_the_sync() {
    let image = image("link-refusal-abandoned");
    let geom = *Fat32::mount(image.device()).expect("probe mount").geometry();
    let bpc = geom.bytes_per_cluster() as usize;
    {
        let (mut fs, mut f, last) = one_cluster_file(&image, bpc);
        let free = fs.free_bytes().expect("free before");
        fs.device().arm(active_link_of(&geom, last));
        let refused = fs.write(&mut f, bpc as u64, &pattern(bpc, 94)).expect_err("armed to refuse");
        assert_eq!(refused, Error::BudgetExpired);
        fs.flush_meta(&mut f, stamp()).expect("flush");
        fs.sync().expect("sync");
        assert_eq!(fs.free_bytes().expect("free after"), free, "the claim was not given back");
        common::assert_fats_agree(&mut fs);
        assert_eq!(read_all(&mut fs, "log.bin"), pattern(bpc, 93));
    }
    image.fsck();
}

/// A directory growing by a cluster is the same claim-then-link, and a refused
/// link there must not strand the zeroed cluster either.
#[test]
fn a_refused_directory_growth_is_redriven_onto_the_cluster_it_claimed() {
    let image = image("dir-growth-refusal");
    let geom = *Fat32::mount(image.device()).expect("probe mount").geometry();
    let per_cluster = geom.bytes_per_cluster() / 32;
    {
        let mut fs = Fat32::mount(RefuseOnceInRange::new(image.device(), IoError::BudgetExpired))
            .expect("mount");
        // The root's one cluster fills after `per_cluster` 8.3 names at most,
        // and the only write to its FAT entry is the link of its growth.
        fs.device().arm(active_link_of(&geom, geom.root_cluster));
        let mut refused = 0;
        for i in 0..per_cluster {
            let name = format!("F{i:04}.BIN");
            if let Err(e) = fs.create(&name, stamp()) {
                assert_eq!(e, Error::BudgetExpired);
                refused += 1;
                fs.create(&name, stamp()).expect("the retry after a spent budget");
            }
        }
        assert_eq!(refused, 1, "the growth's link was armed to refuse once");
        fs.sync().expect("sync");
        common::assert_fats_agree(&mut fs);
        let names = fs.read_dir("", 4096).expect("read_dir");
        assert_eq!(names.iter().filter(|e| e.name.starts_with('F')).count(), per_cluster as usize);
    }
    image.fsck();
}
