...
pub(super) fn syscall_dispatch(num: u64, a1: u64, a2: u64, a3: u64, a4: u64) -> u64 {
    ...
    let result = (move || -> u64 { match num {
        ...
        SYS_DEBUG => match a1 {
            ...
            // Holds each other CPU's acknowledgement back by a2ns in turn and answers with
            // the cheapest wait one of them cost; the arming then stands against every other
            // CPU and outlives the call, so the caller can time a syscall's own shootdown.
