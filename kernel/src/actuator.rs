...
actuators! {
    ...
    /// Offer a claimed function's BAR the largest free run first, with what
    /// firmware answered about its root bridges ignored — the ordering rule
    /// `kernel/src/pcidev` refuses, so the machine's own two probes are what
    /// have to undo it.
