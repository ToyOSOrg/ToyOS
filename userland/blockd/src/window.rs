...
//! thread also reads or writes: a register aperture, a queue the device walks,
//! or a session region the peer and the device share with this process.
//!
...
//! plain write of a doorbell elided, and a plain copy out of an arena block the
//! device is still filling is a data race the compiler may assume away. Every
//! bound is an `assert!`: the build that ships is the one that checks.
...
// A window is an address and a length; what makes an access through it sound
// is the mapping its constructor's contract names, not the thread using it.
...
impl Window {
    ...
    pub fn sub(self, offset: usize, len: usize) -> Self {
        assert!(
            ...
            "blockd: a {len}-byte window at {offset:#x} runs past {:#x}",
            self.len
        );
    ...
    fn at<T>(self, offset: usize) -> *mut T {
        assert!(
            ...
            "blockd: a {}-byte access at {offset:#x} runs past {:#x}",
            size_of::<T>(),
            self.len
        );
        assert!(
            (self.base as usize + offset) % align_of::<T>() == 0,
            "blockd: a {}-byte access at {offset:#x} is not aligned for it",
            size_of::<T>()
        );
    ...
    /// window's base is word aligned: every copy here is of whole blocks and
    /// whole queue entries.
    ...
    fn words(self, offset: usize, len: usize) -> *mut u64 {
        assert!(offset % 8 == 0 && len % 8 == 0, "blockd: a copy of part of a word");
        let whole = self.sub(offset, len);
        assert!(whole.base as usize % align_of::<u64>() == 0, "blockd: a copy from an unaligned window");
        whole.base as *mut u64
    }

    pub fn zero(self) {
        // SAFETY: `self.len` bytes from `self.base`, the whole of what this
        // window covers; `write_bytes` on a pointer the peer may also write is
        // why this is only called before the peer or the device is told of it.
