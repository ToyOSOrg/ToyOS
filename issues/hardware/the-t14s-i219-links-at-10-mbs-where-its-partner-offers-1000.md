...
(PHY address 01, register 25) carries `rev_aneg`, Low Power Link Up (bit 2),
...
`a1000_dis` (bit 6), 1000 Mb/s disabled, whose footnote says its value
"becomes 1b" when `PE_RST_N` goes low and the PHY switches to SMBus. Both take
effect only on a PHY soft reset or on the register's own `Aneg_now` (bit 10),
and both can be set by MDIO write or by "signal toggling" from the MAC.
...
rung `smbus-forced`) and so sets `a1000_dis` by the footnote above.

## What Intel's host driver for this family does about it

Read for hardware facts only: after every PHY reset, and whenever it sets the
power state, it reads OEM Bits, clears `rev_aneg` and `a1000_dis`, sets each
again only from `PHY_CTRL`'s D0a bits (bit 1 and bit 6, both clear on the
T14), and sets `Aneg_now`. It reaches the register at page 768 of PHY address
01, where the datasheet's table prints page 0. At a 1000 Mb/s link it also
changes three PHY registers of its own for this PCH generation (a PLL clock
gate, a K1 clock request, and a pointer gap at page 776 register 20).
...
that its verdict is about frames and not about the 1000 Mb/s settings above,
none of which this driver has, and the page disagreement is one only the part
can settle.
...
before and after the bring-up, writes the D0 bits with `Aneg_now`, and links at
1000 Mb/s full duplex with the lease probe still answering.
