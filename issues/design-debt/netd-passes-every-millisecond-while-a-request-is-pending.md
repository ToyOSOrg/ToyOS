...
a UDP receive or a piped connect is pending, whether or not anything is due.
It is a flat wait standing in for events those requests do not have:
...
`poll_delay`, whose zero is a pass at once. Neither does a lookup: its reply
wakes the NIC and its wait is `resolve::Resolver::wake_in`.
