...
**Exit condition.** The driver reads `VWC` at bring-up and a flush on a
controller that has a volatile write cache issues Flush and waits for its
completion under the caller's operation budget; a test stages a controller
whose Flush fails and sees the fsync fail.
