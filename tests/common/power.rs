...
pub fn quiesce_refuses_a_second_shutdown(
    ...
    let once = |needle: &str| -> Result<usize, String> {
        match at(needle).as_slice() {
            [line] => Ok(*line),
            other => Err(format!("{needle:?} is on {} console line(s), not one\n{whole}", other.len())),
        }
    };
    // **The refusal, by name, inside the window**: after the first call said
    ...
    eprintln!(
        "  [power] the second caller was refused while the first held the stop:\n    {}\n    {}",
...
pub fn blackbox_panic_chain(
    ...
    eprintln!(
        "  [power] the panic crossed the reset, the pass that read it reset in turn, and the \
         pass after that booted a kernel"
    );
    Ok(())
}
...
///
/// The cache write-back every writer of the page also does cannot be judged
/// here at all: this guest is TCG and has no caches for `CLFLUSH` to act on
/// (`issues/panic-path/the-pages-cache-writeback-runs-on-no-guest-this-tree-boots.md`).
