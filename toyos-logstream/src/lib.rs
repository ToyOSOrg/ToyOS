...
//! it. What that buys is a boot whose log arrives *while it is booting* — on
//! the ThinkPad T14 the alternative is a USB stick and a reboot into another
//! operating system.
//!
//! # Why a crate, and what is in it
//!
//! Three parties that share nothing else share these names. The kernel reads
//! [`PARAM`] off the boot parameter line and hands its value on in [`ENV`]; it
//! parses no address and holds no queue. `logd` reads [`ENV`], parses the
//! address with [`endpoint`] and holds the [`Backlog`]; it never sees the
//! parameter line. The build system checks the parameter by name before any
//! image is written and boots no guest at all.
//!
//! # The stream is best effort and the file is not
//!
//! Nothing here can lose a record from the file. [`Backlog::admit`] answers
//! *whether* a line was taken and never waits: a peer that stops taking bytes
//! fills the queue, and the lines that do not fit are refused, counted, and
//! reported in one line — [`Backlog::report`] — which goes into the file like
//! any other. A drop nobody can count is the failure this type exists to make
//! impossible, so `admit`'s refusal and the counter are one statement.
//!
//! Pure: `core` and `alloc`, no `unsafe`, no I/O. Everything a boot could tell
//! us about the accounting is decided here, where a host test costs
//! milliseconds and no guest at all.
...
///
/// **The kernel command line reaches no process**, and giving it one would be a
/// syscall for a string. The kernel already builds `/system/bin/init`'s
/// environment, and an environment is inherited, so the address travels the
/// channel that exists. It is information and not authority: reaching the
/// listener needs a `netd` connector, which one manifest row grants to `logd`
/// and nothing else in the image asks for.
...
/// 21 bytes; the rest is room for a longer form to arrive without this number
...
/// does not carry it.
///
/// The line is comma-separated (`toyos_abi::boot::actuators` splits it the same
/// way); this is spelled here rather than there because the kernel reads it
/// before it has decided anything else about the line, and because the build
/// system asks the same question of an image with no ABI in reach.
...
///
/// **Input that crossed no trust boundary and is still refused rather than
/// guessed at**: the parameter line is the owner's, but a boot whose address is
/// a typo has to say so in its log rather than stream to whatever the typo
/// parsed as.
...
///
/// **One whole `SYS_LOG_READ` batch**, which is the most `logd` can offer
/// between two rounds of its own loop: 64 records at the widest line a record
/// can render. So a listener that misses one round loses nothing, and a
/// listener that has stopped costs this process one batch rather than whatever
/// it felt like.
///
/// It is deliberately the *small* buffer in the chain and not the large one.
/// Everything downstream — the 2 MiB pipe netd reads, netd's own 64 KiB send
/// buffer, the peer's receive window — absorbs a stall of hundreds of kilobytes
/// before this is reached at all: measured on the dev host, 674 KiB went
/// through a peer that was reading nothing without one line being refused. By
/// the time a line reaches this bound every one of those is full, which is a
/// peer that is gone rather than one that is behind.
...
/// always the file's own lines in the file's own order — equal to its first
/// lines when nothing dropped, and the same sequence with holes when something
/// did. Evicting the head instead would keep the end of a boot at the price of
/// making the two readings incomparable.
...
impl Backlog {
    ...
    /// Lines refused for the life of this queue.
    ...
    ///
    /// One line per episode and not one per drop: a peer that takes nothing refuses
    /// thousands in a row, and a line each would be the feedback loop that
    /// starved it.
    pub fn report(&mut self) -> Option<String> {
        ...
        // must arrive at the last line's second one. An accounting that has
        // stopped counting cannot satisfy both.
        Some(alloc::format!(
...
mod tests {
    ...
    fn a_queue_that_is_read_keeps_every_line_in_order() {
        ...
        for i in 0..1000 {
            assert!(q.admit(&std::format!("line {i}\n")));
            assert_eq!(q.take().as_deref(), Some(std::format!("line {i}\n").as_str()));
        }
        assert_eq!(q.dropped(), 0);
        assert_eq!(q.report(), None);
        assert!(q.is_empty());
    }
    ...
    fn a_refusal_says_which_kind_it_is_in_words() {
        for bad in [Malformed::NoPort, Malformed::NotAnAddress, Malformed::NotAPort] {
            assert!(!bad.as_str().is_empty());
        }
        assert_ne!(Malformed::NoPort.as_str(), Malformed::NotAPort.as_str());
    }
