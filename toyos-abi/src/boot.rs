...
pub struct KernelArgs {
    ...
    /// a CPU issues, read off `EFI_PCI_ROOT_BRIDGE_IO_PROTOCOL::Configuration`
    /// before the loader left boot services.
    ///
    ...
    ///
    /// Only the first [`Self::root_bridge_window_count`] entries are firmware's;
    /// the rest are zero.
    pub root_bridge_windows: [RootBridgeWindow; MAX_ROOT_BRIDGE_WINDOWS],
}

/// The most windows the loader will carry. Firmware that names more is refused
/// there by name and this array stays empty: a window list with entries dropped
/// out of it would put a BAR outside an aperture the kernel then had no record
/// of.
pub const MAX_ROOT_BRIDGE_WINDOWS: usize = 16;
...
impl KernelArgs {
    ...
    ///
    /// A count past the array panics here rather than being clamped: the
    /// bootloader is the only writer and it refuses firmware that names more
    /// than the array holds, so a larger count is this project disagreeing with
    /// itself about a layout both halves compile from this file.
